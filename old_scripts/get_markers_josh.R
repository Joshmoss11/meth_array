library(data.table)
BLOOD_METH_THRESHOLD <- 80
BLOOD_UNMETH_THRESHOLD <- 20
OTHER_TISSUES_METH_THRESHOLD <- 80
OTHER_TISSUES_UNMETH_THRESHOLD <- 20
TISSUE_UNMETH_THRESHOLD <- 50
TISSUE_METH_THRESHOLD <- 50
NUM_ALLOWED_BLOOD <- 0
nrows=-1
annot.file <- '/mnt/lustre/hms-01/fs01/joshua.moss/illumina_450k_annotation/Illumina_450k_annotation_josh.txt'
...
for (tissue in tissues.cols){
	...
	unmeth.markers <- merge(annotation,unmeth.markers,by.x=annotation.id,by.y="row.names",sort=F)
	unmeth.markers <- unmeth.markers[order(unmeth.markers$num_methylated_tissues,decreasing=T),]
	unmeth.markers.file <- paste(tissue,'_unmeth.txt',sep='')
	write.table(unmeth.markers,unmeth.markers.file,sep='\t',col.names=NA,quote=F)
	...
	meth.markers <- merge(annotation,meth.markers,by.x=annotation.id,by.y="row.names",sort=F)
	meth.markers <- meth.markers[order(meth.markers$num_unmethylated_tissues,decreasing=T),]
	meth.markers.file <- paste(tissue,'_meth.txt',sep='')
	write.table(meth.markers,meth.markers.file,sep='\t',col.names=NA,quote=F)
